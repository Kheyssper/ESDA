 ...
 * getche -
 *
 * Diferenca entre versao 1 e 2 : Pouca pois nao e impreescindivel
 * usar vetores ao inserir com ao ler estruturas, mas podemos usar
 * quando nao queremos perder dados ou nao queremos retornar algo mais
 * alterar uma variavel, as diferencas somente se fazem sentir nos
 * metodos receberDados
 */
...
void organizaDados(Jogador *jogadores) {
    ...
    Jogador *auxiliador = &jogadores[0];
    Jogador *auxiliador2 = &jogadores[0];
    Jogador *auxiliador3 = &jogadores[1];
    int j;
    int i;
    //auxiliador[i]=jogadores[i];
    //jogadores[i]=jogadores[j];
    //jogadores[j]=auxiliador[i];
    if(aux<2) {//Caso tenha menos que 3 elementos vai dar er	ro
        printf("Errrrrrrrrro");
    } else {
        for(i = 0; i< aux; i++) {
//Aqui onde nos vamos trocar os conteudos dos vectores para organizalo 
//De forma decrescente
            if(auxiliador2->nrDeGolos < auxiliador3->nrDeGolos) {
                auxiliador = auxiliador2;
                auxiliador2 = auxiliador3;
                auxiliador3 = auxiliador;
            }
            auxiliador++;
            auxiliador2++;
            auxiliador3++;
